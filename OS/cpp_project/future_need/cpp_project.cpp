...
class batch {  
    public:
        // string batch_name;
        // vector<priority_queue<process,vector<process>,compMinBT>>system(6);
        // vector<priority_queue<process,vector<process_using_resource>,compMinResReq>>ProcessAllocation;

        // vector<queue<process>>Queue(6);
        // // vector<priority_queue<process,vector<process_using_resource>,compMinResReq>>ProcessAllocation;
        // int RR_row,RR_col;
        ...
        // Containers
        vector<priority_queue<process, vector<process>, compMinBT>> system; // System priority queues
        vector<queue<process>> Queue;                                      // RR Queues
        vector<priority_queue<process, vector<process_using_resource>, compMinResReq>> ProcessAllocation;
...
void store_processes(vector<process>&all_processes,vector<batch>&all_batches){
    cout<<"Inside the store_processes function"<<endl;
    for(int i=0;i<all_processes.size();i++){
        for(int j=0;j<2;j++){
            cout<<"Storing "<<all_processes[i].pid<<endl;
...
void work(vector<batch>&all_batches,vector<core>&all_cores,vector<resource>&all_resources,vector<process*>&all_wokring_processes){
    // assignProcessesToCores(all_batches,all_cores,all_working_processes);
}
