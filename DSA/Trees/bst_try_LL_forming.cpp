    ...
    if(data<t->data){
        if(!t->left){
            ...
            if(t->left->data<data){
               ...
               one_node->left=t->left;
               t->left=one_node;
               cout<<"Rearranged and "<<data<<" places to left of "<<t->data<<endl;
               return;
            }
            else add(data,t->left);
        }
    } else {
        if(!t->right){
            ...
            if(data<t->right->data){
                cout<<"Linking "<<data<<" with "<<t->right->data<<endl;
               node* one_node=new node(data);
               one_node->right=t->right;
               t->right=one_node;
               return;
            } else {
                add(data,t->right);
...
void delete_nodes(node*root){
    ...
    if(root->left!=nullptr)cout<<"On the left i have : "<<root->left->data<<endl;
    else cout<<"On the left i ahve null"<<endl;
    if(root->right!=nullptr)cout<<"On the right i have : "<<root->right->data<<endl;
    else cout<<"On the right i ahve null"<<endl;
